# BAIT Framework — Mauboussin

**Source**: Michael Mauboussin, "The Success Equation" and related research.
Applied as one cell in the Verdict table of every ticker page; a lens is justified in §7 only where its rating is non-obvious.

---

## What It Is

BAIT identifies *why* a stock might be mispriced relative to intrinsic value.
It is not a valuation tool — it is a signal-detection tool. The more BAIT
factors that overlap on a single name, the higher the conviction that a
genuine inefficiency exists. It never sets a verb or a zone (CLAUDE.md R8);
the hurdle does that.

---

## The Four Lenses

### B — Behavioral Inefficiency

The market is overreacting emotionally to a narrative, fear, or recent event
in a way not supported by the underlying operating data.

**Key questions:**
- Is there an identifiable sentiment catalyst (earnings overreaction, narrative rotation, index exclusion, CEO departure fear)?
- Is the fear empirically supported or not yet visible in operating data?
- What would it take for the fear to be validated? Is that happening?
- Is short interest elevated? (>10% = elevated, >20% = extreme)

**Signal strength**: Strong if fear is narrative-driven but contradicted by reported financials.

---

### A — Analytical Inefficiency

Consensus models are missing or underweighting something that a careful
reader of primary sources (filings, transcripts) can see clearly.

**Common sources of analytical edge:**
- Buyback-driven EPS compounding (shares retiring faster than revenue growth)
- AI or automation-driven cost leverage not yet in consensus margin models
- Attach rate acceleration across a product suite
- Hidden balance sheet optionality (real estate, tax assets, subsidiary value)
- FCF capex cycle normalization (elevated capex masking true earning power)
- Take rate expansion mismodeled as flat
- Incremental ROIC far above or below what the multiple implies (see `compounding.md`)

**Key questions:**
- What does the price-implied expectations line in §5 say the market assumes, and is it beatable?
- What is consensus EPS in 2 years, and is that beatable? By how much?
- Is there a specific financial line item that media coverage and sell-side summaries routinely skip?

**Signal strength**: Strong if the gap between model and consensus is >20% on a key metric.

---

### I — Informational Asymmetry

A piece of primary data exists that changes the picture — but most investors
rely on media summaries and miss it.

**Where informational edge hides:**
- Earnings transcript Q&A (not covered in press release summaries)
- Conference presentations (specific unit economics or pilot data disclosed)
- SEC footnotes (lease obligations, contingent liabilities, related-party transactions)
- Proxy statement (incentive metrics, option grant timing)
- Investor day slide decks (long-term targets not in consensus models)

**Key questions:**
- Did management disclose something in the transcript that media didn't cover?
- Is there a data point in the 10-K that changes the moat or growth story?
- What do you know from reading primary sources that you couldn't get from Bloomberg?

**Signal strength**: Strong if the informational gap is specific and quantifiable.

---

### T — Technical Factor

Mechanical, non-fundamental price pressure creates a temporary mis-entry point.
This lens is **informational only**: it can sharpen timing inside an entry zone the
hurdle already opened, never open one.

**Common technical catalysts:**
- High short interest → squeeze potential on any positive data
- Index inclusion/exclusion (forced buying or selling)
- Institutional ownership approaching a floor (forced sellers exhausted)
- Buyback acceleration at current price level (management as floor buyer)
- Options expiry dynamics (gamma pinning, dealer hedging)
- Tax-loss selling season (November–December)
- Stock split or reverse split signal effects

**Signal strength**: Strong if multiple technical factors converge at the same price level.

---

## BAIT Verdict

| Overlap | Signal |
|---------|--------|
| 1 factor | Weak — interesting, not high conviction |
| 2 factors | Moderate — a real setup worth the full page |
| 3 factors | Strong — high conviction that a mispricing exists |
| 4 factors | Very Strong — maximum conviction; prioritize the research |

Conviction is about the *existence* of a mispricing. Whether to act, and at what price, is the hurdle's job (R8). Sizing of any kind is out of scope (R3).

**Template verdict format** (Verdict table cell, expanded in §7 only if non-obvious):
> "WING presents a B+A+T triple overlap: [1-sentence on B], [1-sentence on A], [1-sentence on T].
> Informational (I) signal is moderate. Overall: High conviction."

---

## BAIT Applied — Wiki Examples

| Ticker | B | A | I | T | Verdict | Date |
|--------|---|---|---|---|---------|------|
| BKNG | ✓ AI disintermediation fear | ✓ FCF compounding undermodeled | ✓ "9/9/15" framework underappreciated | ✓ Buyback floor | Triple+ overlap | Mar 2026 |
| WING | ✓ SSS narrative fear | ✓ Franchise royalty misframed as restaurant | ✓ SSS stabilization in transcript | ✓ 13% short interest + buyback | Triple+ overlap | Mar 2026 |
| LLY | — | ✓ GLP-1 TAM undermodeled | ✓ Pipeline depth underappreciated | — | Analytical-primary | Mar 2026 |
| UNH | ✓ MLR spike fear | ✓ Managed care cycle not in models | ✓ Earnings call tone shift | — | Double overlap | Mar 2026 |

*Table updated by LLM agent after each full thesis update.*
