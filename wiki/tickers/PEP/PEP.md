# PEP — PepsiCo, Inc.

**Schema**: v2.14
**Last Updated**: 2026-05-30
**Status**: Active
**Live Price**: $144.19 (2026-05-30; last close May 29, 2026) — [Yahoo Finance](https://finance.yahoo.com/quote/PEP/)

---

## Summary

**Thesis**: PEP is a Dividend King (54 consecutive annual increases) and the only major CPG with simultaneous #1 positions in U.S. salty snacks and beverages, now trading near the low end of its 52-week range at a ≈17× forward core P/E and a ≈4.05% yield — its highest in years — because the market is pricing GLP-1 snack-demand destruction, two years of Frito-Lay North America volume weakness, MAHA artificial-dye reformulation, and Elliott activist pressure as structural rather than cyclical; Q1 FY2026's return to FLNA volume growth (+2% on Lay's), accelerating international (EMEA +18%, LatAm +16%), and a productivity program targeting 100+ bps of margin expansion through 2028 argue the fear is overshooting a wide-moat compounder.
🟢 **Non-holder**: Initiate (scaled) · 🟡 **Holder**: Hold-Add

### Scenario Table (5-year terminal, FY2031E)

| 52-wk range | Now (+%ile) | Bear | Entry | Base / PW EV | Trim | Bull | R/R |
|---|---|---|---|---|---|---|---|
| $127.60 – $171.48 | $144.19 (~38%) | $127 | <$150 | $180 / **$183** | $183 – $225 | $225 | **≈4.7:1** |

### KPI Strip

| BAIT | Moat | FY26E P/E (core) | Div Yield | FCF Yield | Next catalyst |
|---|---|---|---|---|---|
| **Moderate-High** (B+A+I) | **Wide** | ≈17× | **4.05%** ($1.48/qtr) | ≈3.9% | Q2 FY2026 earnings (~mid-July 2026) |

### Why / Why not / Next read

🟢 **Why**:
- Dividend King with 54 consecutive annual increases at a ≈4.05% yield (highest in years) + ≈3.9% FCF yield = ≈8% cash-return floor with 0.39 beta defensive characteristics, near the low end of the 52-wk range
- Q1 FY2026 returned FLNA to +2% volume growth (Lay's, +4% occasions) while international accelerated (EMEA +18%, LatAm Foods +16%) — early evidence the structural-snacks-decline narrative is overshooting
- PBNA segment margin collapsed to 3.9% (from 9.4% in FY2023) — productivity program (20% SKU cut, 3 plant closures, 100+ bps margin expansion target through 2028) gives outsized mean-reversion optionality consensus may under-credit

⚠️ **Why not**:
- GLP-1 weight-loss drugs are a genuine multi-year overhang on the highest-margin segment (FLNA ~60% U.S. salty-snack share); chip spending down ≈11% among GLP-1 users — more exposed than any food peer
- MAHA / FDA synthetic-dye phase-out by end-2026 forces reformulation of core Frito-Lay brands — COGS and brand-disruption risk that is not yet priced
- Elliott (~$4B stake) engagement is constructive today, but a second consecutive year of missing the 2–4% organic target raises the probability of forced structural alternatives (a beverages/snacks split)

📅 **Next read**: Q2 FY2026 earnings (~mid-July 2026) — does FLNA volume recovery extend, does international stay strong, and does the productivity walk show up in PBNA margin; sustained progress + reaffirmed/raised guide would firm conviction toward unscaled Initiate

---

## Business Overview

PepsiCo, Inc. (NASDAQ: PEP) is one of the world's largest food and beverage companies, assembled through three defining transactions: the 1965 Pepsi-Cola/Frito-Lay merger, the 1998 Tropicana acquisition, and the 2001 Quaker Oats/Gatorade merger. **PEP is the only major consumer-packaged-goods company holding simultaneous #1 positions in both U.S. salty snacks (Frito-Lay) and a top-two position in U.S. beverages.** FY2025 net revenue was $93.9B. Its brand portfolio spans Lay's, Doritos, Cheetos, Tostitos, Ruffles (Frito-Lay); Quaker, Gatorade, Tropicana stakes; and Pepsi, Mountain Dew, Mtn Dew, Aquafina, Bubly, Rockstar, and Starbucks ready-to-drink (beverages). Headquartered in Purchase, NY.

**Business model**: Brand-premium pricing on everyday snacks and beverages, distributed largely through PepsiCo's own direct-store-delivery (DSD) network of 300,000+ routes. The "Power of One" advantage bundles snacks + beverages into a single shelf-space and promotional package no single-category peer can match. Snacks/foods are ≈55–57% of revenue but carry far higher margins (FLNA ≈22–26% operating margin) than beverages (PBNA ≈4–9%), so the snacks business drives the majority of profit. Revenue is subscription-like in character — consumers re-purchase continuously — giving extreme stability but limiting high-growth potential.

**Current leadership**: Ramon Laguarta has been CEO since October 2018 (also Chairman). Steve Schmitt became CFO effective November 10, 2025, recruited from Walmart U.S. (where he was EVP/CFO). In 2025, Elliott Investment Management disclosed a ≈$4B stake and engaged constructively; PepsiCo responded with a shareholder-value plan (productivity acceleration, board refreshment, portfolio focus) that Elliott publicly endorsed.

---

## Pivotal Investment Question

> **Is PepsiCo's 2024–2026 de-rating (from ≈$196 in 2023 to ≈$127–$150) a cyclical overreaction to GLP-1 fear + temporary Frito-Lay volume softness + activist noise, or does it reflect genuine structural impairment of snacking demand and pricing power that justifies a permanently lower (≈14–15×) multiple?**

Q1 FY2026 offered the first real counter-evidence: Frito-Lay North America returned to +2% volume growth (Lay's specifically), international segments accelerated sharply (EMEA +18%, LatAm Foods +16% reported), and management reaffirmed the FY2026 organic +2–4% / mid-single-digit core EPS growth guide while CFO Schmitt detailed a productivity program targeting 100+ bps of operating-margin expansion through 2028. But the bear levers are real and not yet resolved: GLP-1 adoption is a multi-year demand question with PEP the most exposed food name; the FDA/MAHA synthetic-dye phase-out forces reformulation of core brands by end-2026; and the PBNA beverage segment's margin collapse to 3.9% (from 9.4% in FY2023) is the single largest drag on consolidated profitability. The question resolves over the next 2–4 earnings cycles: if FLNA volume holds positive, international stays strong, and the productivity walk lifts PBNA margins back toward 6–8%, the multiple re-rates toward 18–20×. If volume reverts and GLP-1 erosion accelerates, the structural thesis wins at 14–15×.

---

## Key Stats Snapshot

| Metric | Value | Notes |
|--------|-------|-------|
| Live Price | **$144.19** | May 29, 2026 close — [Yahoo Finance](https://finance.yahoo.com/quote/PEP/) |
| 52-Week High | $171.48 | February 12, 2026 |
| 52-Week Low | $127.60 | June 26, 2025 |
| % From 52-Wk High | –15.9% | |
| % Above 52-Wk Low | +13.0% | Lower third of range |
| Market Cap | ≈$197B | 1.37B shares × $144.19 |
| EV (est.) | ≈$237B | Market cap + ≈$40B net debt (FY2025) [*Estimate*] |
| FY2025 Revenue | $93.9B | Ended December 27, 2025 |
| TTM Revenue (Q1 FY2026) | ≈$95.4B | Trailing 12 months ending Mar 21, 2026 |
| FY2025 GAAP EPS | $6.00 | Down from $6.95 (FY2024) on one-time charges |
| FY2025 Core EPS | $8.14 | Roughly flat vs. $8.16 (FY2024) |
| FY2026 Core EPS Growth Guide | mid-single-digit (≈+5%) | ≈$8.55 implied [*Management guidance*] |
| FY2026 Organic Sales Guide | +2% to +4% | Affirmed at Q1 |
| Q1 FY2026 GAAP EPS | $1.70 | +27% YoY; beat |
| Q1 FY2026 Core EPS | $1.61 | +9% YoY; beat consensus $1.55 |
| Q1 FY2026 Organic Growth | +2.6% | Reported revenue +8.5% (FX + M&A tailwinds) |
| Forward P/E (FY2026E core) | ≈17× | On ≈$8.55 core EPS |
| P/E (TTM, GAAP) | ≈24× | On $6.00 GAAP EPS (one-time-charge depressed) |
| **Quarterly Dividend** | **$1.48/share** | **Declared May 6, 2026; 54th consecutive annual increase** |
| Dividend Yield | ≈4.05% | ≈$5.92 annualized at $144.19 |
| Consecutive Div Increases | 54 years | Dividend King |
| Buyback Authorization | $10B through Feb 28, 2030 | New program announced Feb 3, 2026 |
| Beta | 0.39 | Defensive |
| Short Interest | ≈1.82% of float | ≈24.9M shares; 4.5 days to cover; negligible — [MarketBeat](https://www.marketbeat.com/stocks/NASDAQ/PEP/short-interest/) |

---

## Section 1 — Annual Financial Metrics

### Annual Trend (time in columns)

| Metric | FY2021 | FY2022 | FY2023 | FY2024 | FY2025 | TTM (Q1 FY2026) |
|--------|--------|--------|--------|--------|--------|-----------------|
| Net Revenue | $79.5B | $86.4B | $91.5B | $91.9B | $93.9B | ≈$95.4B |
| YoY Revenue Growth | — | +8.7% | +5.9% | +0.4% | +2.3% | — |
| Gross Margin | 53.4% | 53.0% | 54.2% | 54.6% | 54.2% | — |
| Operating Income | $11.2B | $11.5B | $12.0B | $12.9B | $11.5B | — |
| Operating Margin | 14.0% | 13.3% | 13.1% | 14.0% | 12.2% | — |
| Net Income | $7.6B | $8.9B | $9.1B | $9.6B | $8.2B | — |
| EPS (GAAP diluted) | $5.49 | $6.42 | $6.56 | $6.95 | $6.00 | — |
| Core EPS | $6.26 | $6.73 | $7.62 | $8.16 | $8.14 | ≈$8.27 |
| Free Cash Flow | $7.0B | $5.6B | $7.9B | $7.2B | $7.7B | $8.8B |
| Organic Sales Growth | ≈8% | ≈14% | ≈10% | ≈2% | ≈1% | — |
| Dividends Paid | ≈$5.8B | ≈$6.2B | ≈$6.7B | ≈$7.2B | $7.6B | — |
| Share Repurchases | ≈$0.1B | ≈$1.5B | ≈$1.0B | ≈$1.0B | $1.0B | — |

*Sources: [stockanalysis.com — PEP Financials](https://stockanalysis.com/stocks/pep/financials/); [PEP FY2025 10-K (SEC EDGAR)](https://www.sec.gov/Archives/edgar/data/0000077476/000007747626000007/pep-20251227.htm); core EPS and organic growth from earnings 8-Ks*

**Key multi-year read**:
- Organic growth decelerated sharply from the pricing-led peak of ≈14% (FY2022) and ≈10% (FY2023) to ≈2% (FY2024) and ≈1% (FY2025) — the end of the inflation pass-through era and the onset of volume softness, especially in Frito-Lay North America.
- FY2025 GAAP EPS fell to $6.00 (from $6.95) on one-time charges (impairments/restructuring); **core EPS was essentially flat at $8.14** — the cleaner operating read. FY2025 also carried a ≈5pp FX headwind to core EPS.
- Operating margin compressed to 12.2% in FY2025 (–180bps), the lowest of the window, driven primarily by the PBNA beverage segment (see §2). This is the explicit target of the 2026–2028 productivity program.
- FCF is durable but in a lower band than revenue scale implies ($5.6–7.9B). FY2025 FCF of $7.7B (8.2% margin, 93% conversion) covered the $7.6B dividend with little cushion — the buyback is funded largely by incremental cash and debt capacity, not FCF surplus.

### Cash Flow (3-year)

| Metric | FY2023 | FY2024 | FY2025 |
|--------|--------|--------|--------|
| Operating Cash Flow | $13.4B | $12.5B | $12.1B |
| Capex | $5.5B | $5.3B | $4.4B |
| Free Cash Flow | $7.9B | $7.2B | $7.7B |
| FCF Margin | 8.7% | 7.8% | 8.2% |
| FCF Conversion (FCF/NI) | 87% | 75% | 93% |

*Source: [stockanalysis.com — PEP cash flow](https://stockanalysis.com/stocks/pep/financials/cash-flow-statement/)*

**Balance sheet snapshot**: Cash ≈$9.2B; total debt ≈$49.2B; net debt ≈$40.0B (FY2025). Diluted shares ≈1.367B (Jan 23, 2026). Capex falling (<5% of revenue guide) supports FCF conversion ≥80%.

### Quarterly Trend (last 8 quarters)

| Metric | Q2 FY24 | Q3 FY24 | Q4 FY24 | Q1 FY25 | Q2 FY25 | Q3 FY25 | Q4 FY25 | Q1 FY26 |
|--------|---------|---------|---------|---------|---------|---------|---------|---------|
| Revenue | $22.5B | $23.3B | $27.8B | $17.9B | $22.7B | $23.9B | $29.3B | $19.4B |
| Net Income | $3.1B | $2.9B | $1.5B | $1.8B | $1.3B | $2.6B | $2.5B | $2.3B |
| EPS (GAAP) | $2.23 | $2.13 | $1.11 | $1.33 | $0.92 | $1.90 | $1.85 | **$1.70** |
| Core EPS | — | — | — | $1.48 | — | — | $2.26 | **$1.61** |

*Source: [stockanalysis.com — PEP quarterly](https://stockanalysis.com/stocks/pep/financials/?p=quarterly); core EPS from Q1/Q4 8-Ks*

**Q1 FY2026 note** (12 weeks ended March 21, 2026; reported April 16, 2026): Revenue $19.4B (+8.5% reported / +2.6% organic — reported boosted by ≈+3.4pp FX and ≈+2.5pp net M&A); GAAP EPS $1.70 (+27%); **Core EPS $1.61 (+9%), beat consensus $1.55**; operating income $3.2B (+24%, margin 16.5% +70bps). FLNA volume returned to +2%. FY2026 guidance affirmed.

---

## Section 2 — Revenue Mix & Geographic Split

### Segment Overview (new 6-segment structure, effective Q1 2025)

Effective Q1 2025 PepsiCo realigned to six reportable segments. FY2023–FY2024 below are shown on the restated new basis.

| Segment | FY2025 Net Rev | % Total | FY2025 Op. Profit | FY2025 Margin | Key Brands |
|---------|---------------|---------|-------------------|---------------|-----------|
| PepsiCo Beverages NA (PBNA) | $28.2B | 30.0% | $1.09B | **3.9%** | Pepsi, Mtn Dew, Gatorade, Aquafina, Bubly |
| PepsiCo Foods NA (PFNA) | $27.5B | 29.3% | $6.17B | **22.4%** | Lay's, Doritos, Cheetos, Tostitos, Quaker |
| EMEA | $18.0B | 19.2% | $2.11B | 11.7% | Walkers, Doritos, Pepsi (Europe/MEA) |
| LatAm Foods | $10.5B | 11.2% | $2.01B | 19.1% | Sabritas, Gamesa, snacks |
| Intl Beverages Franchise | $5.0B | 5.3% | $1.77B | 35.4% | Franchise/concentrate |
| Asia Pacific Foods | $4.6B | 4.9% | $0.37B | 8.0% | Snacks (APAC) |
| **Total** | **$93.9B** | 100% | **$13.5B** | **14.4%** | |

*Source: [stock-analysis-on.net — PEP segments](https://www.stock-analysis-on.net/NASDAQ/Company/PepsiCo-Inc/Ratios/Reportable-Segments) (derived from 10-K); [PEP FY2025 10-K](https://www.sec.gov/Archives/edgar/data/0000077476/000007747626000007/pep-20251227.htm)*

**The single most important segment fact**: **PBNA operating margin collapsed from 9.4% (FY2023) → 8.3% (FY2024) → 3.9% (FY2025)** — North American beverages went from ~$2.6B to ~$1.1B in segment profit, the largest drag on consolidated margin. This is partly mix/cost/charge-driven and is the explicit focus of the productivity program — it is both the biggest risk and the biggest mean-reversion opportunity. Conversely, the **International Beverages Franchise margin tripled to 35.4%** (capital-light concentrate model), and PFNA snacks remain the profit engine at 22.4%.

**Q1 FY2026 segment growth** (reported): PFNA $6.33B (+2%, organic +1%, volume +2%); PBNA $6.39B (+9%); EMEA $2.82B (+18%); LatAm Foods $1.93B (+16%); Asia Pacific Foods $1.14B (+11%). *Source: [Q1 2026 8-K](https://www.sec.gov/Archives/edgar/data/77476/000007747626000019/q120268-kxexhibit991.htm)*

### Geographic Revenue Split

- **United States**: ≈56% of revenue (≈$52.6B FY2025)
- **International**: ≈44% (≈$41.3B FY2025) — EUR, BRL, MXN, GBP, plus an EM basket
- FY2025 FX was a ≈1pp headwind to revenue and ≈5pp to core EPS; FY2026 FX is expected to flip to a modest ≈+1pp tailwind.

*Source: derived from [FY2025 10-K segment composition](https://www.sec.gov/Archives/edgar/data/0000077476/000007747626000007/pep-20251227.htm)*

### Forward Revenue Mix Shifts

- **Snacks vs. beverages**: snacks/foods ≈55–57% of revenue but a clear majority of profit; the profit mix tilts further to snacks if PBNA margins stay depressed.
- **Functional / better-for-you**: Doritos Protein, Pepsi Prebiotic Cola, Smartfood FiberPop, plus the Poppi ($1.95B) and Siete Foods ($1.2B) acquisitions shift mix toward health-positioned, gut-health, and multicultural categories.
- **International acceleration**: EMEA, LatAm, and APAC are the growth engines; no demand softness observed internationally per Q1 FY2026 commentary.

---

## Section 3 — Competitive Moat & Landscape

**Moat Rating: Wide** (Morningstar — [source](https://www.morningstar.com/stocks/this-dividend-aristocrat-is-buyheres-why))

### Moat Sources

- **Brand equity** (Strong): Lay's, Doritos, Cheetos, Gatorade, and Pepsi are category-defining global brands. Frito-Lay holds an estimated ≈60% of the U.S. salty-snack market (*[Estimate, Food Business News](https://www.foodbusinessnews.net/articles/26375-slowing-salty-snack-sales-hit-pepsicos-frito-lay)*) — a concentration no peer approaches.
- **Direct-store-delivery (DSD) scale** (Strong): 300,000+ DSD routes give PepsiCo 2–4% lower out-of-stock rates than warehouse-delivered peers and direct control of shelf placement and in-store execution. DSD is expensive to operate but extremely hard to replicate (*[Food Dive](https://www.fooddive.com/news/as-cpg-distribution-shifts-pepsico-reaffirms-its-preference-for-dsd/436989/)*).
- **"Power of One" scale** (Strong): Bundling snacks + beverages into a single shelf and promotional package for retailers is a structural advantage versus single-category competitors (KO in beverages, MDLZ/GIS in snacks).
- **Procurement and manufacturing scale** (Moderate-Strong): A $93.9B revenue base yields purchasing leverage on corn, potatoes, oils, aluminum, and PET resin.
- **Pricing power** (Strong historically; stressed currently): Mid-single-digit price increases funded the FY2022–FY2023 organic surge but came at the cost of volume; FY2025 required price reductions on some snacks to restore volume — the FY2026 challenge is volume-led recovery without ceding the price base.

**Moat vulnerabilities**:
- GLP-1 weight-loss drugs threaten the highest-margin, highest-share category (salty snacks) more directly than any peer.
- Private label gains share fastest in value/discount channels (Aldi, Lidl, dollar stores) where the DSD advantage thins.
- Persistent premium pricing during inflation accelerated private-label trial; quality parity is narrowing in some snack and beverage sub-categories.

### Competitive Landscape

| Competitor | Revenue | Market Position | Key Moat / Threat vs. PEP |
|-----------|---------|-----------------|---------------------------|
| **Coca-Cola (KO)** | ≈$47B | Global #1 in CSDs (≈20% U.S. vs. PEP ≈16%) | Asset-light bottler model, higher beverage margins; primary threat in beverages (where PEP is weakest) |
| **Mondelez (MDLZ)** | ≈$36B | Global biscuits + chocolate | Limited U.S. salty-snack overlap; pushing into well-being snacks; strong EM snacking |
| **Keurig Dr Pepper (KDP)** | ≈$15B | U.S. CSD #3 + at-home brewing | Pressures PBNA on flavor innovation and the at-home channel |
| **Kellanova (K)** | ≈$13B | Pringles + cereal/snacks | Pringles is a direct salty-snack competitor; Mars acquisition adds scale |
| **General Mills (GIS)** | ≈$20B | Broad CPG, health snacks | Nature Valley, Fiber One compete in better-for-you snacking |
| **Private label** | — | ≈20–30% share in value channels | Most acute structural threat; accelerates during multi-year premium-price cycles |

*Sources: Revenue estimates from company disclosures [*Estimate*]; [Hudson Labs — PEP competitors](https://www.hudson-labs.com/research/pepsico-competitors-pep-key-rivals-in-2026); [FourWeekMBA](https://fourweekmba.com/pepsi-competitors/)*

**How PEP's moat differs**: PepsiCo is the only major CPG with simultaneous #1 (or top-two) positions in both U.S. salty snacks and beverages, distributed through its own DSD network. KO has a stronger, higher-margin beverage franchise but no snack business; MDLZ/K/GIS have snacks but no beverage scale or DSD. The "Power of One" cross-category package is the genuine differentiator. **Tail risk**: GLP-1 is the first credible threat that attacks the *category demand* for PepsiCo's two core occasions (salty snacking, sugary/diet beverages) simultaneously — a different and more existential vector than the private-label margin pressure PEP has historically absorbed.

---

## Section 4 — Management & Leadership

### CEO and Executive Team

**Ramon Laguarta** (Chairman & CEO since October 2018): Joined PepsiCo in 1996; first Spanish CEO of a major U.S. multinational; MBA from ESADE. Career spans European and global commercial leadership. His tenure has delivered consistent organic growth but the post-2023 volume slowdown, beverage-margin erosion, and a multi-year share-price underperformance vs. the S&P 500 attracted Elliott's activist engagement in 2025.

**Steve Schmitt** (CFO, effective November 10, 2025): Recruited from Walmart U.S. (EVP/CFO since 2016); prior Yum! Brands and UPS. Predecessor Jamie Caulfield retired after 30+ years and served as adviser through May 2026. Schmitt's external operating-and-retail background is significant — his early commentary has centered on productivity, SKU rationalization, plant closures, and margin expansion, signaling a more cost-and-returns-disciplined CFO posture.

- Q1 FY2026 (Apr 16): Core EPS $1.61 (+9%), beat consensus $1.55; FLNA volume +2%; FY2026 guide affirmed — a confidence-supportive print.
- CEO quote (Q1 FY2026): *"Nothing has changed for us to give you guys a different guidance on how we see the business evolving."* — reaffirmed 2–4% organic, with the upper end expected in H2 2026.
- CFO quote (Q1 FY2026): *"We're benefiting from some of the moves from last year — the reduced headcount, plant closures, reduction in SKU count."* — record productivity savings targeted FY2026; 100+ bps margin expansion over 2026–2028.

**Capital allocation track record**:
- **Dividend King**: 54 consecutive annual increases; quarterly dividend raised to $1.48/share (declared May 6, 2026); ≈$5.92 annualized (≈4.05% yield). Decade dividend CAGR ≈7%.
- New **$10B buyback authorization** through Feb 28, 2030 (announced Feb 3, 2026); recent buyback pace modest (~$1B/year) relative to the dividend.
- ~$7.6B dividends + ~$1B buybacks in FY2025 — capital return tilts heavily toward the dividend.
- M&A: Siete Foods ($1.2B, closed Jan 2025) and Poppi ($1.95B, 2026) — bolt-on bets on better-for-you and functional categories.

### Recent Management Commentary — Primary Source Synthesis

*Sources: [PEP Q1 2026 earnings call transcript — Motley Fool](https://www.fool.com/earnings/call-transcripts/2026/04/16/pepsico-pep-q1-2026-earnings-call-transcript/); [PepsiCo shareholder-value priorities / Elliott / 2026 outlook — PepsiCo IR](https://www.pepsico.com/en/newsroom/press-releases/2025/pepsico-announces-priorities-to-enhance-shareholder-value-and-provides-preliminary-2026-outlook)*

- **On Frito-Lay volume**: *"We grew volume this quarter in Lay's in particular… we grew volume 2%. We grew occasions, units 4%."* — FLNA added ~300M new consumption occasions YoY. Investment relevance: the single most important data point against the structural-decline bear case; one quarter, not yet a trend.
- **On international**: *"The international business is very solid, continues to accelerate… we haven't seen an impact on demand."* — Investment relevance: ≈44% of revenue is showing no GLP-1 / volume issue; the snacking-demand concern is a U.S.-specific debate, not global.
- **On health trends / dye removal**: *"Consumers are coming back to the category… now we're offering solutions with no colors, no artificial flavors."* — Investment relevance: PEP is framing MAHA reformulation as a demand opportunity, but the COGS and execution risk is real (see §6).
- **On GLP-1** (prior commentary): Laguarta has framed GLP-1 as presenting *"more opportunities than threats"*, expecting fiber to be *"the next protein."* Investment relevance: management's posture is offense (functional-snack innovation), not retreat — but this is an unproven bet against a structural demand question. *[Source: [Food Business MEA](https://www.foodbusinessmea.com/pepsico-ceo-says-glp-1-weight-loss-drugs-present-more-opportunities-than-threats-for-snacks-giant/)]*
- **On productivity (CFO Schmitt)**: record productivity savings in FY2026; 100+ bps operating-margin expansion 2026–2028 via SKU rationalization (≈20% U.S. SKU cut), three plant closures, and reduced headcount. Investment relevance: the explicit lever to reverse the PBNA margin collapse and lift consolidated margin from the 12.2% FY2025 trough.

**Activist context — Elliott Investment Management**: Elliott disclosed a ≈$4B stake in 2025 and engaged constructively. PepsiCo announced a shareholder-value plan (productivity acceleration, board refreshment, portfolio focus) that Elliott's Marc Steinberg publicly endorsed — no hostile proxy contest. This is a thesis-relevant overhang in both directions: constructive pressure accelerates the margin agenda, but a second year of missing the organic-growth target raises the probability of forced structural alternatives (a snacks/beverages separation).

---

## Section 5 — Strategic Growth Initiatives

1. **Productivity & margin expansion (2026–2028)**: ≈20% U.S. SKU reduction, three plant closures, reduced headcount, record targeted productivity savings, and a 100+ bps operating-margin expansion goal. This is the central self-help lever to reverse the FY2025 margin trough (12.2%) and the PBNA collapse to 3.9%. *Source: [PepsiCo IR](https://www.pepsico.com/en/newsroom/press-releases/2025/pepsico-announces-priorities-to-enhance-shareholder-value-and-provides-preliminary-2026-outlook); [Bakery & Snacks](https://www.bakeryandsnacks.com/Article/2026/03/24/pepsico-cuts-skus-and-bets-on-protein-snacks-in-major-2026-strategy-reset/)*

2. **Functional / better-for-you portfolio**: Doritos Protein (10g/serving), Pepsi Prebiotic Cola, Smartfood FiberPop, SunChips Fiber, and "Simply"/no-artificial-color-or-flavor versions of Cheetos and Doritos. The explicit hedge against GLP-1 and health-trend demand shifts.

3. **Poppi acquisition (≈$1.95B, 2026)**: Prebiotic soda (apple-cider vinegar + prebiotics, ≈5g sugar/can) — entry into the fast-growing functional/gut-health soda category to compete with Olipop and revitalize the beverage portfolio. *Source: [Marketing Dive](https://www.marketingdive.com/news/pepsico-buys-poppi-2B-pepsi-purchase-prebiotic-healthy-soda-better-for-you-drink-olipop-simply-pop/742679/)*

4. **Siete Foods (≈$1.2B, closed Jan 2025)**: Grain-free, Mexican-American heritage brand extending FLNA into cleaner-label, multicultural snacking.

5. **International acceleration**: EMEA, LatAm Foods, and APAC are the growth engines (Q1 FY2026 EMEA +18%, LatAm +16% reported); management sees no demand softness internationally.

6. **Away-from-home / foodservice**: Named by Laguarta as a key Q1 FY2026 FLNA volume driver — a channel less exposed to retail private-label competition.

7. **AI + DSD optimization**: Generative AI integrated into the 300,000+ route DSD network for real-time inventory and route optimization — a margin and service-level lever leveraging PEP's structural distribution advantage. *Source: [Yahoo Finance](https://finance.yahoo.com/news/pepsico-three-channel-distribution-network-170022730.html)*

---

## Section 6 — Key Risks

*Materiality filter applied per Rule #25: risks below are differentiated from peers, tied to thesis-break triggers, or tied to large discretionary bets. Generic boilerplate excluded.*

| Risk | Impact | Probability | Priced In? | Notes |
|------|--------|-------------|-----------|-------|
| **GLP-1 demand destruction** | High | Moderate | Partially | Chip spending down ≈11% among GLP-1 users; Barclays/Morgan Stanley model ≈3–5% U.S. calorie reduction by 2030. PEP is the most exposed food name (≈60% U.S. salty-snack share). The single largest structural overhang; *not fully priced* if adoption accelerates. ([Indigrowth](https://www.indigrowth.com/weight-loss-pills-are-eating-snack-sales/)) |
| **FLNA volume weakness / price elasticity** | High | Moderate | Partially | FLNA organic declined in FY2025; price cuts required in early 2026 to restore volume. Q1 FY2026 +2% volume is an early recovery signal — needs 2–3 quarters of confirmation. ([Marketplace](https://www.marketplace.org/story/2026/02/03/pepsico-slashes-prices-on-snacks-in-bid-to-boost-sales/)) |
| **PBNA beverage-margin impairment** | High | Moderate | Not priced | PBNA operating margin collapsed 9.4% → 3.9% (FY2023→FY2025). If structurally driven (mix, cost, competitive pricing vs. KO/KDP) rather than fixable via productivity, consolidated margin cannot recover and the ≈17× multiple is hard to defend. *Not fully priced* — the productivity walk is the swing factor. |
| **MAHA / synthetic-dye regulation** | Moderate | Moderate-High | Not priced | FDA/HHS targeting phase-out of synthetic dyes by end-2026. PEP is reformulating core Frito-Lay brands — COGS increases and brand-disruption / consumer-acceptance risk. ([Quench Agency](https://quenchagency.com/rfk-jr-and-the-end-of-artificial-dyes-is-reshaping-the-aisle/)) |
| **Elliott-driven structural pressure** | Moderate | Moderate | Not priced | ≈$4B stake, constructive today. A second consecutive year of missing the 2–4% organic target raises the probability of forced alternatives (beverages/snacks split). Market currently treats engagement as benign. ([PepsiCo IR](https://www.pepsico.com/en/newsroom/press-releases/2025/pepsico-announces-priorities-to-enhance-shareholder-value-and-provides-preliminary-2026-outlook)) |
| **Private-label trade-down** | Moderate | Moderate | Partially | Structural in value/discount channels (Aldi, Lidl, dollar) where DSD advantage thins. Less acute than GLP-1 but persistent. |
| **Input cost / tariff volatility** | Moderate | Moderate | Partially | Corn, potatoes, oils, aluminum, PET resin exposure; trade-policy uncertainty adds tail risk to COGS. |
| **FX headwinds** | Moderate | Moderate-High | Partially | ≈44% of revenue is international. FY2025 was a ≈5pp core-EPS headwind; FY2026 expected ≈+1pp tailwind — a swing factor that can flip quickly on dollar strength. |

**5-Year Risk Factor Evolution (synthesis)**: PepsiCo's risk narrative shifted materially across FY2021–FY2025. The FY2021–FY2023 emphasis on commodity/input-cost inflation and supply-chain disruption (the pricing-power era) gave way by FY2024–FY2025 to demand-side risks: consumer trade-down, volume elasticity to multi-year price increases, and — newly prominent — the GLP-1 and "health and wellness" / regulatory (added-sugar, synthetic-dye) cluster that questions long-run category demand for salty snacks and sugary beverages. The FY2025 segment realignment and the PBNA margin collapse moved beverage competitiveness from a footnote to a primary concern. The arc shows management recognizing that the post-2023 challenge is structural-demand and margin-mix, not transitory cost — which is precisely why the 2026 plan pivots from pricing to productivity and portfolio reshaping. *Sources: [FY2025 10-K](https://www.sec.gov/Archives/edgar/data/0000077476/000007747626000007/pep-20251227.htm); [FY2023 10-K](https://www.sec.gov/Archives/edgar/data/0000077476/000007747624000008/pep-20231230.htm)*

---

## Section 7 — Industry-Specific Macro Analysis

**Consumer staples sector dynamics**: PEP is a defensive staple (beta 0.39) that typically outperforms in recessions and lags bull markets. FY2024–FY2026 is atypical: PEP has underperformed even within staples because the headwinds are partly intrinsic (volume softness, beverage-margin erosion, GLP-1 exposure) rather than purely macro.

**GLP-1 adoption (highest-impact structural macro factor)**: The dominant multi-year variable. Rising GLP-1 penetration reduces caloric intake, with early data showing snack and CSD consumption falling among users. A 1–2 standard-deviation faster adoption curve could compress U.S. snacking volume by mid-single digits by 2030 — disproportionately hitting FLNA, PEP's profit engine. The bull counter is that PEP's functional-snack innovation (protein, fiber) and away-from-home channel capture offset the at-home decline.

**Health / regulatory (MAHA)**: The FDA/HHS synthetic-dye phase-out by end-2026 is a near-term operational macro factor — reformulation costs and consumer-acceptance risk across core Frito-Lay brands. Added-sugar scrutiny and potential soda taxes are slower-moving regulatory tails for PBNA.

**Beverage competitive intensity**: KO and KDP are pressuring PBNA on innovation and channel; the segment's 3.9% margin reflects both internal cost issues and external competition. Recovery depends as much on category positioning as on productivity.

**FX and EM consumer**: ≈44% international means EUR/BRL/MXN/GBP moves materially affect reported results; a strong dollar is a recurring headwind. EM consumer strength (LatAm, AMESA, APAC) is currently a tailwind offsetting U.S. softness.

**Interest-rate sensitivity**: At ≈17× forward core P/E and a ≈4% yield, PEP is moderately rate-sensitive as a bond-proxy staple. Rate cuts would support the multiple and the dividend's relative attractiveness; sustained high rates cap re-rating.

---

## Section 8 — Valuation & Comparable Analysis

### Multiples Summary

| Metric | Value | Notes |
|--------|-------|-------|
| Current Price | $144.19 | May 29, 2026 |
| Market Cap | ≈$197B | |
| EV | ≈$237B | [*Estimate*; net debt ≈$40B] |
| P/E (TTM, GAAP) | ≈24× | $6.00 GAAP EPS (one-time-charge depressed) |
| P/E (TTM, core) | ≈17.4× | ≈$8.27 TTM core EPS |
| P/E (FY2026E core) | ≈17× | ≈$8.55 core EPS [*Management guidance*] |
| EV/EBITDA | ≈15–16× | [*Estimate*] |
| Dividend Yield | ≈4.05% | $1.48/quarter; ≈$5.92 annualized |
| FCF Yield | ≈3.9% | ≈$7.7B FY2025 FCF / ≈$197B market cap |
| P/E (5-yr historical range, core) | ≈17–25× | Peak ≈25× (2020–2023); trough ≈17× today |

### Peer Comparison

| Company | Forward P/E (core) | Dividend Yield | Organic Growth | Commentary |
|---------|-------------------|----------------|----------------|------------|
| **PEP** | ≈17× | ≈4.05% | +2–4% guide | Trough multiple, highest yield in years; GLP-1/beverage-margin overhang |
| Coca-Cola (KO) | ≈22× | ≈2.9% | Mid-single | Premium for cleaner beverage-only model, higher margin, less snack/GLP-1 risk |
| Mondelez (MDLZ) | ≈19× | ≈2.8% | Mid-single | Snack-focused, strong EM; cocoa-cost headwind |
| Procter & Gamble (PG) | ≈21× | ≈2.95% | In-line to +4% | Higher-multiple staples peer; see [PG page](../PG/PG.md) |

*Sources: [stockanalysis.com — PEP](https://stockanalysis.com/stocks/pep/) [*Analyst consensus*]; peer multiples [*Estimate*, May 2026]*

### Valuation Assessment

**At $144.19 (≈17× FY2026E core)**: PEP trades at a multi-year trough multiple and a discount to KO (≈22×), MDLZ (≈19×), and PG (≈21×), with the highest dividend yield (≈4.05%) in years. The discount is rational — PEP carries the most direct GLP-1 exposure, a depressed beverage segment, and an unresolved activist situation. But the gap arguably over-discounts a wide-moat, Dividend-King compounder that is showing early FLNA volume recovery and accelerating international growth, and has a clear self-help margin lever.

**Downside scenario (≈14–15×)**: If GLP-1 erosion accelerates and PBNA margins don't recover, the multiple compresses toward 14×. At 14.5× FY2026E ≈$8.55 ≈ **$124**.

**Upside scenario (≈19–20×)**: If FLNA volume holds positive, productivity lifts margin 100+ bps, and the GLP-1 fear proves overdone, PEP re-rates toward its historical ≈19–20× and the dividend compounds. At 19× FY2028E ≈$9.7 ≈ **$184**; the 5-year terminal bull extends further (see §11).

**Entry discipline**: At $127–$150, FCF yield ≈3.9–4.5% + dividend yield ≈4.0–4.6% = ≈8% cash-return floor with defensive (0.39 beta) characteristics and a 54-year dividend-growth streak. A reasonable entry for income-and-quality-oriented buyers, with optionality on a margin/volume recovery.

---

## Section 9 — Catalyst & Sentiment Tracker

### Live Price & Positioning

- **Price**: $144.19 (May 29, 2026 close — [Yahoo Finance](https://finance.yahoo.com/quote/PEP/))
- **52-week range**: $127.60 – $171.48
- **% from 52-wk high**: –15.9%
- **% above 52-wk low**: +13.0%
- **Market cap**: ≈$197B
- **Dividend yield**: ≈4.05% (Dividend King; $1.48/quarter, ≈$5.92 annualized)
- **Beta**: 0.39 (defensive)
- **Short interest**: ≈1.82% of float (≈24.9M shares, 4.5 days to cover; +4.7% MoM) — negligible; weakness is long-holder fatigue, not a short attack ([MarketBeat](https://www.marketbeat.com/stocks/NASDAQ/PEP/short-interest/))

### Analyst Consensus

*Sources: [stockanalysis.com — PEP Forecast](https://stockanalysis.com/stocks/pep/forecast/); [stockanalysis.com — PEP Ratings](https://stockanalysis.com/stocks/pep/ratings/)*

- **Rating breakdown**: 4 Strong Buy / 4 Buy / 14 Hold / 0 Sell / 1 Strong Sell = 23 analysts
- **Consensus**: Hold
- **Median target**: $173 | **Average**: $171.05 | **High**: $195 | **Low**: $132
- **Implied upside from $144.19**: +19.7% to median

**Recent rating actions (since ~March 2026)**:
- **Piper Sandler (Lavery, May 20, 2026)**: Reiterated Buy; target $181
- **Wells Fargo (Carey, May 18, 2026)**: Maintained Hold; cut target $165 → $160
- **Citi (Falorni, May 15, 2026)**: Maintained Buy; target $182
- **Barclays (Lieberman, Apr 20, 2026)**: Reiterated Hold; target $154 → $158
- **Morgan Stanley (Mohsenian, Apr 17, 2026)**: Maintained Hold; cut target $180 → $158
- **UBS (Grom, Apr 17, 2026)**: Maintained Buy; target $186
- **J.P. Morgan (Teixeira, Apr 17, 2026)**: Maintained Buy; raised target $172 → $178
- **Evercore ISI (Ottenstein, Apr 17, 2026)**: Maintained Hold; raised target $165 → $170

**Pattern**: A split tape — Buy-rated firms (UBS $186, Citi $182, Piper $181, JPM $178) see meaningful upside, while Hold-rated firms (Morgan Stanley, Wells Fargo, Barclays) trimmed targets toward $158–160 post-Q1. Median $173 implies ≈+20%. The Hold-heavy consensus reflects the GLP-1/beverage-margin uncertainty; no Buy upgrade cycle yet.

### Insider Activity (Last 90 Days)

*Source: [SEC EDGAR](https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0000077476&type=4)*

- No open-market purchases or material discretionary sales confirmed from primary source for Mar–May 2026.
- Form 3 (initial ownership) filed for David Gibbs (director nominee) — [SEC EDGAR](https://www.sec.gov/Archives/edgar/data/0000077476/000162828026032944/davidgibbs-section16poas.htm).
- Routine RSU vesting / tax-withholding sales likely exist but unconfirmed (OpenInsider/Fintel inaccessible this pass) — *[link pending]*.

### Delivered Catalysts ✅

- **[2026-05-06] [Dividend Raise] ✅** — 💰 Quarterly dividend declared $1.48/share; 54th consecutive annual increase; ex-div June 5, payable June 30, 2026. ([PRNewswire](https://www.prnewswire.com/news-releases/pepsico-declares-quarterly-dividend-302764461.html))
- **[2026-05-06] [Shareholder Meeting] ✅** — Annual meeting: all 13 directors re-elected; say-on-pay approved; ESG/governance proposals rejected. ([Yahoo Finance](https://finance.yahoo.com/markets/stocks/articles/pepsico-agm-shareholders-elect-13-021647423.html))
- **[2026-04-16] [Earnings Q1 FY2026] ✅** — Revenue $19.4B (+8.5% reported / +2.6% organic), beat; Core EPS $1.61 (+9%), beat $1.55; FLNA volume +2%; international accelerating; FY2026 guidance affirmed (organic +2–4%). Thesis status: Cautiously Strengthened. ([SEC 8-K](https://www.sec.gov/Archives/edgar/data/77476/000007747626000019/q120268-kxexhibit991.htm))

### Recent Corporate News (Last 90 Days)

- **[2026-05-20] [Analyst]** — Piper Sandler reiterated Buy, $181 ([stockanalysis.com](https://stockanalysis.com/stocks/pep/ratings/))
- **[2026-05-18] [Analyst]** — Wells Fargo cut target $165 → $160 (Hold)
- **[2026-05-06] [Dividend / AGM] ✅** — See above
- **[2026-04-17] [Analyst]** — Post-Q1 cluster: Morgan Stanley –$22 (Hold $158); JPM +$6 (Buy $178); UBS held $186 (Buy)
- **[2026-04-16] [Earnings Q1 FY2026] ✅** — Delivered. See above
- **[2026-02-04] [Dividend / Buyback]** — New $10B buyback authorization (through Feb 2030); dividend activity. ([StockTitan](https://www.stocktitan.net/news/PEP/pepsi-co-declares-quarterly-qf26x7p0q6pr.html))
- **[2026-02-12] [Price action]** — 📈 52-week high $171.48 (defensive rotation + tariff-relief narrative)
- **[2025-06-26] [Price action]** — 📉 52-week low $127.60

### Upcoming Catalysts

| Date | Event | Why It Matters |
|------|-------|---------------|
| 📅 **June 5, 2026** | **Ex-dividend** ($1.48/share) | Income mechanics |
| 📅 **June 30, 2026** | **Dividend payment** | Income floor |
| 📅 **~Mid-July 2026** | **Q2 FY2026 earnings** | Does FLNA volume recovery extend; international durability; PBNA margin / productivity walk; FY2026 guide confirmation |
| Ongoing | GLP-1 adoption + scanner data | Multi-year structural demand question for FLNA/PBNA |
| Ongoing | MAHA / FDA synthetic-dye rulemaking | Reformulation cost + timeline through end-2026 |
| Ongoing | Elliott engagement | Constructive today; escalation risk if organic target missed again |

---

## Section 10 — BAIT Framework

### B — Behavioral: **Strong**

A Dividend King with a 54-year increase streak is trading near the low end of its 52-week range at a ≈4.05% yield — its highest in years — after multi-year underperformance versus the S&P 500. The fear narrative is loud and multi-pronged: GLP-1 "eating snack sales," Frito-Lay volume weakness, MAHA dye reformulation, and an activist publicly pressuring a "broken" company. Each fear has a real fundamental kernel, but the aggregate has pushed a wide-moat compounder to a trough multiple and a yield that historically marks attractive entry. The behavioral signal is strong: this is a high-quality, defensive name priced as if its category is in secular decline, when Q1 FY2026 (FLNA volume +2%, international accelerating) provides early evidence the decline is being over-extrapolated.

### A — Analytical: **Moderate-Strong**

The PW EV (≈$183) sits ≈27% above spot ($144.19) on a 5-year terminal basis, plus a ≈4% dividend — a ≈9–10%/yr total-return setup with asymmetric R/R (≈4.7:1). The analytical edge is in the productivity program and PBNA: beverage operating margin collapsed to 3.9% (from 9.4%), so even partial mean reversion to 6–7% adds ~$1.3–1.7B of segment profit — a meaningful consolidated-margin and EPS lever consensus Hold models may under-credit. Conversely, the genuine analytical uncertainty is the terminal GLP-1 impact on FLNA volume, which is hard to model and bounds the downside.

### I — Informational: **Moderate**

The Q1 FY2026 primary data — FLNA volume +2% (Lay's), +4% occasions, international with "no impact on demand" — is the first genuinely positive informational signal against the media GLP-1/Frito-doom narrative. CFO Schmitt's specific productivity detail (SKU cuts, plant closures, 100+ bps margin target) and Elliott's *constructive* (not hostile) framing are both underappreciated versus headline summaries. One quarter is not yet an informational edge; it becomes one if Q2 confirms the volume turn and the margin walk shows up.

### T — Technical: **Weak-Moderate**

- Short interest negligible (≈1.82%) — no squeeze catalyst.
- Stock near the lower third of the 52-wk range with technical support around the $127 low.
- Dividend (ex-div June 5) provides minor mechanical support; the ≈4% yield attracts income buyers as a soft floor.
- No index rebalancing or forced-buying event imminent.

### BAIT Verdict

**Moderate-High signal — B+A+I triple overlap (B-Strong, A-Moderate-Strong, I-Moderate).** The behavioral mispricing is the strongest lens: a Dividend King at a trough multiple and peak-in-years yield, with early fundamental counter-evidence (Q1 FLNA volume) and a clear analytical margin lever (PBNA mean reversion + productivity). T remains weak. This is a stronger setup than the typical staples name — not a quadruple, but a genuine triple-overlap contrarian-quality entry, gated on Q2 FY2026 confirmation of the volume/margin turn.

---

## Section 11 — Bull / Bear / Base Cases

*Scenarios use a 5-year terminal horizon (FY2031E) per Rule #26. Probabilities sum to 100%.*

| Scenario | Conditions | Price Range | Midpoint | Probability |
|----------|-----------|-------------|----------|-------------|
| 🟢 **Bull** | FLNA volume holds positive; GLP-1 impact proves manageable; productivity lifts operating margin 100–150bps; PBNA recovers toward 7–8%; international compounds; multiple re-rates to ≈19–20×; FY2031E core EPS ≈$12; dividend grows ≈7%/yr | $215–$240 | $225 | 30% |
| 🟡 **Base** | Organic 2–3%; modest margin expansion; PBNA partial recovery; GLP-1 a slow grind, not a cliff; multiple holds ≈16.5–17.5×; FY2031E core EPS ≈$10.9; dividend grows ≈5–6%/yr | $170–$195 | $180 | 50% |
| 🔴 **Bear** | GLP-1 erosion accelerates; FLNA volume reverts negative; PBNA margin stays depressed; MAHA reformulation disrupts core brands; multiple compresses to ≈14×; FY2031E core EPS ≈$9.2; dividend growth slows to 1–3%/yr | $115–$135 | $127 | 20% |

*Bull anchored on margin recovery (PBNA mean reversion + productivity) and multiple re-rating toward historical norms. Bear anchored on structural GLP-1 demand impairment + permanent beverage-margin damage. Base assumes the muddle-through that current Hold consensus implies.*

---

## Section 12 — Probability-Weighted Expected Value

| Scenario | Midpoint | Probability | Weighted |
|----------|----------|-------------|---------|
| 🟢 Bull | $225.00 | 30% | $67.50 |
| 🟡 Base | $180.00 | 50% | $90.00 |
| 🔴 Bear | $127.00 | 20% | $25.40 |
| **PW EV (5-yr terminal)** | | **100%** | **≈$182.90** |

| Current Price | PW EV | Price Return | + Dividend (≈4%/yr) | Total Return (annualized proxy) |
|--------------|-------|-------------|--------------------|-----------------------|
| $144.19 | ≈$183 | **+26.8%** (5-yr) | ≈+4%/yr | **≈9–10%/yr** |

**R/R (Bull / Bear)**: ($225 – $144.19) / ($144.19 – $127) = $80.81 / $17.19 ≈ **4.7:1** — attractive asymmetry, helped by a ≈4% yield and a 54-year dividend-growth floor cushioning the downside.

**Interpretation**: At $144, PEP offers a ≈9–10%/yr total-return setup (price + dividend) with R/R ≈4.7:1 — better asymmetry than most staples because the multiple is at a trough, the yield is at a multi-year high, and there is a tangible margin-recovery lever (PBNA + productivity) that consensus Hold models under-credit. The bear case is real (GLP-1) but bounded by the wide moat, the cash-return floor, and the low starting multiple. This is a quality-at-a-discount entry, not a deep-value screamer — the asymmetry rests on the GLP-1 fear proving cyclical/overdone, which Q2 FY2026 will help adjudicate.

---

## Section 13 — Recommendation & Bottom Line

**Thesis in one sentence**: PEP is a wide-moat Dividend King (54 straight raises) trading near its 52-week low at a ≈17× trough multiple and a ≈4.05% peak-in-years yield because the market is pricing GLP-1 snack-demand destruction, Frito-Lay volume weakness, and a depressed beverage segment as structural — but Q1 FY2026's return to FLNA volume growth, accelerating international, and a credible 100+ bps margin-expansion program argue the fear is overshooting a high-quality compounder.

**For a non-holder**: 🟢 **Initiate (scaled) at $127–$150** — buy a starter position into the trough multiple + ≈4% yield, with the next tranche reserved for Q2 FY2026 confirmation of the FLNA volume turn and PBNA margin walk. R/R ≈4.7:1 and a ≈9–10%/yr total-return setup justify entry now; the GLP-1 uncertainty argues for scaling rather than a full position pre-Q2.

**For a current holder**: 🟡 **Hold-Add** — the dividend and trough valuation support the floor; add on weakness below $140. Trim only into the $183–$225 zone if the recovery narrative runs ahead of the volume/margin evidence.

🎯 **Attractive entry zone**: **$127–$150** — rationale: ≈16–17.5× FY2026E core P/E; ≈4.0–4.6% dividend yield (multi-year high); ≈3.9–4.5% FCF yield; near 52-wk low technical support; ≈8% cash-return floor; R/R ≈4.7:1; ≈20% discount to PW EV $183

🎯 **Trim zone**: **$183–$225** — rationale: PW EV < price < Bull scenario; ≈21–24× FY2026E core P/E re-rates toward the upper end of the historical range and requires full execution proof on volume + margin

🔴 **Exit / avoid zone**: **>$225** — would imply a ≈19–20× multiple on a fully-recovered FY2031E EPS with no margin of safety; only justified if GLP-1 fear fully resolves and PBNA margin durably recovers

**Thesis-break triggers** ⚠️:
- ⚠️ FLNA volume reverts to negative in Q2/Q3 FY2026 after Q1's +2% — signals the Q1 turn was noise
- ⚠️ GLP-1 erosion shows up as accelerating U.S. snack/CSD volume declines in scanner data
- ⚠️ PBNA operating margin fails to recover off the 3.9% trough through FY2026 — productivity program not working
- ⚠️ FY2026 organic growth tracks below the +2% guide floor — second consecutive year of missing
- ⚠️ Elliott escalates from constructive engagement to a proxy contest or forces a structural split on bad terms
- ⚠️ MAHA dye reformulation materially disrupts core Frito-Lay brand volumes or COGS beyond plan
- ⚠️ Dividend growth cut below ≈3% or freeze (very low probability; 54-year streak; would signal a serious FCF problem)

📅 **Next review trigger**: **Q2 FY2026 earnings (~mid-July 2026)** — the critical confirmation print. Sustained FLNA volume + durable international + visible PBNA margin recovery + reaffirmed/raised guide → upgrade toward unscaled Initiate. Volume reversion or accelerating GLP-1 erosion → step back toward Watch.

---

### Sources

- [PEP FY2025 Form 10-K — SEC EDGAR](https://www.sec.gov/Archives/edgar/data/0000077476/000007747626000007/pep-20251227.htm)
- [PEP FY2025 Form 10-K — IR PDF](https://investors.pepsico.com/docs/pepsico-5v9wci20/media/Files/investors/q4-2025-form-10k.pdf)
- [PEP Q4 + FY2025 Earnings PR — SEC 8-K](https://www.sec.gov/Archives/edgar/data/0000077476/000007747626000009/q420258-kxexhibit991.htm)
- [PEP Q1 FY2026 Earnings PR — SEC 8-K](https://www.sec.gov/Archives/edgar/data/77476/000007747626000019/q120268-kxexhibit991.htm)
- [PEP Q1 FY2026 10-Q — SEC EDGAR](https://www.sec.gov/Archives/edgar/data/77476/000007747626000017/pep-20260321.htm)
- [PEP FY2024 10-K — SEC EDGAR](https://www.sec.gov/Archives/edgar/data/77476/000007747625000007/pep-20241228.htm)
- [PEP FY2023 10-K — SEC EDGAR](https://www.sec.gov/Archives/edgar/data/0000077476/000007747624000008/pep-20231230.htm)
- [PEP Q1 2026 Earnings Call Transcript — Motley Fool](https://www.fool.com/earnings/call-transcripts/2026/04/16/pepsico-pep-q1-2026-earnings-call-transcript/)
- [PEP Q4 2025 Prepared Management Remarks — IR PDF](https://investors.pepsico.com/docs/pepsico-5v9wci20/media/Files/investors/q4-2025-prepared-management-remarks.pdf)
- [PepsiCo Shareholder-Value Priorities / Elliott / 2026 Outlook — PepsiCo IR](https://www.pepsico.com/en/newsroom/press-releases/2025/pepsico-announces-priorities-to-enhance-shareholder-value-and-provides-preliminary-2026-outlook)
- [stockanalysis.com — PEP Financials](https://stockanalysis.com/stocks/pep/financials/)
- [stockanalysis.com — PEP Cash Flow](https://stockanalysis.com/stocks/pep/financials/cash-flow-statement/)
- [stockanalysis.com — PEP Quarterly](https://stockanalysis.com/stocks/pep/financials/?p=quarterly)
- [stockanalysis.com — PEP Analyst Forecast](https://stockanalysis.com/stocks/pep/forecast/)
- [stockanalysis.com — PEP Ratings](https://stockanalysis.com/stocks/pep/ratings/)
- [stock-analysis-on.net — PEP Reportable Segments](https://www.stock-analysis-on.net/NASDAQ/Company/PepsiCo-Inc/Ratios/Reportable-Segments)
- [Yahoo Finance — PEP Quote](https://finance.yahoo.com/quote/PEP/)
- [MarketBeat — PEP Short Interest](https://www.marketbeat.com/stocks/NASDAQ/PEP/short-interest/)
- [PRNewswire — PepsiCo May 2026 Dividend Declaration](https://www.prnewswire.com/news-releases/pepsico-declares-quarterly-dividend-302764461.html)
- [StockTitan — PepsiCo Dividend / Buyback](https://www.stocktitan.net/news/PEP/pepsi-co-declares-quarterly-qf26x7p0q6pr.html)
- [Yahoo Finance — PepsiCo AGM May 2026](https://finance.yahoo.com/markets/stocks/articles/pepsico-agm-shareholders-elect-13-021647423.html)
- [Morningstar — PepsiCo Wide Moat](https://www.morningstar.com/stocks/this-dividend-aristocrat-is-buyheres-why)
- [Food Dive — PepsiCo DSD advantage](https://www.fooddive.com/news/as-cpg-distribution-shifts-pepsico-reaffirms-its-preference-for-dsd/436989/)
- [Food Business News — Frito-Lay volume slowdown](https://www.foodbusinessnews.net/articles/26375-slowing-salty-snack-sales-hit-pepsicos-frito-lay)
- [Marketplace — PepsiCo snack price cuts](https://www.marketplace.org/story/2026/02/03/pepsico-slashes-prices-on-snacks-in-bid-to-boost-sales/)
- [Bakery & Snacks — 2026 strategy reset](https://www.bakeryandsnacks.com/Article/2026/03/24/pepsico-cuts-skus-and-bets-on-protein-snacks-in-major-2026-strategy-reset/)
- [Marketing Dive — Poppi acquisition](https://www.marketingdive.com/news/pepsico-buys-poppi-2B-pepsi-purchase-prebiotic-healthy-soda-better-for-you-drink-olipop-simply-pop/742679/)
- [Indigrowth — GLP-1 snack demand destruction](https://www.indigrowth.com/weight-loss-pills-are-eating-snack-sales/)
- [Food Business MEA — Laguarta on GLP-1](https://www.foodbusinessmea.com/pepsico-ceo-says-glp-1-weight-loss-drugs-present-more-opportunities-than-threats-for-snacks-giant/)
- [Quench Agency — MAHA / synthetic dye phase-out](https://quenchagency.com/rfk-jr-and-the-end-of-artificial-dyes-is-reshaping-the-aisle/)
- [StockTitan — Steve Schmitt CFO appointment](https://www.stocktitan.net/news/PEP/pepsi-co-announces-new-chief-financial-nx1y7kwb3w91.html)
- [Hudson Labs — PEP competitors](https://www.hudson-labs.com/research/pepsico-competitors-pep-key-rivals-in-2026)
- raw/PEP/filings/financials-notes.md (local; financials + segment synthesis)
- raw/PEP/press-releases/market-sentiment-notes.md (local; live data + sentiment)
- raw/PEP/transcripts/qualitative-notes.md (local; management + moat + risks)
