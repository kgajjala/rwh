# SHOP — Shopify Inc.

> **Schema**: v2.1
> **Last Updated**: 2026-04-17
> **Live Price Verified**: $125.83 at April 17, 2026 (Yahoo Finance JSON API)
> **Ticker Type**: Capital-light platform / commerce infrastructure

---

## Business Overview

Shopify Inc. (NASDAQ/TSX: SHOP) is the world's largest independent e-commerce platform, founded in 2006 by Tobias Lütke after he tried to sell snowboards online and could not find an adequate platform. Shopify provides the essential internet infrastructure for retail businesses of any size: online storefronts, POS hardware, payments (Shopify Payments / Shop Pay), shipping, capital/lending, B2B wholesale, and — increasingly — the connective tissue between merchants and AI agents via the Universal Commerce Protocol (UCP). It powers millions of merchants in 175+ countries, processed $378B in GMV in FY2025, and commands 14%+ U.S. e-commerce share.

Revenue splits between **Merchant Solutions** (~73% of FY2025 revenue: payments, shipping, capital, advertising) and **Subscription Solutions** (~27%: SaaS plans, Plus enterprise upgrades). Shopify owns no inventory, runs no warehouses, and carries no retail risk — pure platform economics. Recent enterprise wins include General Motors, L'Oréal, Estée Lauder, Sonos, Keurig Dr. Pepper, and Amer Sports (Wilson, Salomon).

---

## Moat Assessment

**Wide and Widening** — switching costs + network effects + emerging AI-commerce standard-setter position.

**Sources**:
- **Switching costs (primary)**: Online store + payments + POS + shipping + lending + advertising + AI storefronts are fully integrated. 11+ consecutive quarters of 25%+ revenue growth (ex-logistics) demonstrates retention durability at enterprise scale.
- **Platform network effects**: 875M+ unique shoppers; Shop Pay creates consumer-side lock-in; Shop App native GMV grew +94% YoY in 2025.
- **Data / AI advantage**: $378B GMV across millions of merchants — one of the most diverse commerce datasets globally. Powers Catalog's specialized LLMs and Sidekick.
- **First-mover in agentic commerce**: Universal Commerce Protocol (UCP) co-developed with Google, endorsed by Visa, Mastercard, Stripe, Adyen, AmEx, Target, Walmart, Etsy, Wayfair, and 20+ others. OpenAI's retreat from "Instant Checkout" (only ~30 merchants successfully onboarded; routes traffic to Shopify storefronts instead) is the cleanest validation of fulfillment complexity flowing *through* Shopify, not around it.

**Vulnerabilities**:
- High valuation multiple is the moat's tax — any growth deceleration below 20% would compress 84× P/E violently
- Amazon Buy with Prime is a credible — if narrow — competitive alternative for some merchants
- Macro / tariff exposure on cross-border SMB merchants (de minimis removal)
- Dual-class governance (Lütke 40% voting via Founder Share) is feature-now, risk-later

---

## Pivotal Investment Question

> Does AI disrupt Shopify (LLM tools let merchants self-build, or AI agents bypass checkout entirely) — or does AI amplify Shopify (multi-channel, multi-currency, multi-agent commerce complexity makes Shopify's infrastructure MORE necessary, and UCP/Catalog become the connective tissue between merchants and every AI surface)?

The market priced the *disruption* scenario through Q4 2025/Q1 2026 (drove the stock from $182 to $88). Primary data — 15× growth in AI-originated orders since Jan 2025, UCP co-developed with Google, OpenAI's Instant Checkout retreat — supports the *amplification* scenario. The thesis is "AND" not "OR": AI surfaces discover, Shopify handles fulfillment. Recovery from $88 to $126 suggests the market is starting to agree, but the re-rating is mid-flight.

---

## Key Stats Snapshot

| Metric | Value | Notes |
|--------|-------|-------|
| Live Price | **$125.83** | April 17, 2026 (Yahoo Finance) |
| 52-Week High | $182.19 | early 2025 |
| 52-Week Low | $88.14 | recent low; recovered |
| % From 52-Wk High | –30.9% | |
| % Above 52-Wk Low | +42.8% | strong recovery off the low |
| Market Cap | ~$163B | ~1.30B diluted shares × $125.83 |
| FY2025 Revenue | $11.56B | +30% YoY |
| FY2025 GMV | $378B | +29% YoY |
| FY2025 Operating Income | $1.47B | 12.7% op margin |
| FY2025 GAAP EPS | $1.36 | |
| FY2025 FCF | ~$2.0B | 17.3% FCF margin |
| Net Cash | ~$4.6B | |
| Payments Penetration | 68% of GMV | up from 64% |
| Trailing P/E | ~93× | at $125.83 |
| FY2026E P/E | ~85× | $1.48 consensus EPS |
| Buyback Authorization | $2.0B | first-ever capital return; announced Feb 2026 |
| Q1 2026 Earnings Date | **~May 2026** | hard catalyst |

---

## Section 1 — Why Does This Company Exist? + Pivotal Investment Question

**Founding insight (Tobias Lütke, 2004–2006)**: There was no adequate internet platform for an entrepreneur to sell snowboards online. So Lütke built one. That store became Shopify. The mission has been consistent for two decades: provide essential internet infrastructure for retail businesses of any size, lowering the barrier to entrepreneurship.

**Today**: World's largest independent e-commerce platform. Millions of merchants in 175+ countries. $378B GMV in 2025. 14%+ U.S. e-commerce share. Platform-of-platforms spanning online stores, POS, payments, shipping, capital, lending, B2B wholesale, and AI commerce infrastructure.

**In Buffett terms**: Shopify is the toll booth on entrepreneurial commerce. Subscription fee + percentage of every transaction. No inventory, no warehouses, no retail risk.

**Pivotal question**: Does AI disrupt or amplify the platform? At $125.83 (recovered from $88, still 31% off highs), the market is pricing *partial* disruption — the cleanest read is that primary data (UCP, OpenAI retreat, AI-order growth) increasingly supports the amplification thesis, but the re-rating is incomplete.

---

## Section 2 — Annual Financial Metrics

### Revenue, GMV & Attach Rate

| Metric | 2020 | 2021 | 2022 | 2023 | 2024 | 2025 | 2026E |
|--------|------|------|------|------|------|------|-------|
| Revenue ($B) | $2.93 | $4.61 | $5.60 | $7.06 | $8.88 | $11.56 | ~$15.0 |
| Revenue Growth | +86% | +57% | +21% | +26% | +26% | +30% | ~+30% |
| GMV ($B) | $120 | $175 | $197 | $236 | $292 | $378 | ~$475 |
| Attach Rate (Rev/GMV) | 2.44% | 2.63% | 2.84% | 2.99% | 3.04% | 3.06% | ~3.15% |

**Attach rate flywheel**: Revenue consistently grows faster than GMV via Payments penetration (68%), Shop Pay (+62%), shipping, capital, and advertising (Shop Campaigns revenue doubled in 2025). Even if GMV growth slows to 15–20%, revenue can grow 25%+ via attach-rate expansion alone.

### Profitability

| Metric | 2022 | 2023 | 2024 | 2025 | 2026E |
|--------|------|------|------|------|-------|
| Gross Margin | 49.3% | 49.8% | 50.7% | 48.2% | ~47.5% |
| Operating Income ($B) | –$0.82 | $0.27 | $1.08 | $1.47 | ~$2.1 |
| Operating Margin | –14.6% | 3.8% | 12.2% | 12.7% | ~14% |
| GAAP Diluted EPS | –$0.35 | $0.14 | $0.99 | $1.36 | ~$1.48 |
| Headcount | ~10,000 | ~8,300 | ~8,100 | ~7,600 | ~7,600 |

### Cash Flow & Capital Return

| Metric | 2023 | 2024 | 2025 |
|--------|------|------|------|
| Free Cash Flow ($B) | $0.84 | $1.62 | $2.0 |
| FCF Margin | 11.9% | 18.2% | 17.3% |
| SBC / % Rev | $550M / 7.8% | $546M / 6.1% | ~$570M / 4.9% |
| Net Cash | ~$4.1B | ~$4.5B | ~$4.6B |
| Buyback | — | — | $2.0B authorized Feb 2026 |

### Recent Quarterly Trend

| Quarter | Revenue | Growth YoY | GMV | FCF Margin |
|---------|---------|------------|-----|------------|
| Q4 2024 | $2.81B | +31% | $94.5B | 22% |
| Q1 2025 | $2.36B | +27% | $74.8B | 15% |
| Q2 2025 | $2.70B | +31% | $87.8B | 16% |
| Q3 2025 | $2.84B | +32% | $92.0B | 18% |
| Q4 2025 | $3.67B | +31% | $123.8B | 19.5% |
| **FY2025** | **$11.56B** | **+30%** | **$378B** | **17%** |

11 consecutive quarters of 25%+ revenue growth (ex-logistics). 5-year revenue CAGR ~31.6% at $11.6B scale — among the fastest-growing large-cap technology companies globally.

---

## Section 3 — Geographic Revenue Mix

| Region | FY2025 Share |
|--------|-------------|
| United States | ~63% |
| EMEA | ~21% |
| APAC | ~10% |
| Canada | ~5% |
| LatAm | ~1% |

International revenue grew +36% in FY2025 (vs. +33% FY2024 — accelerating). Nearly half of merchant base is outside North America. Europe GMV +49% YoY. Payments launched in 16 new regions in 2025. Forward expectation: international gradually compresses U.S. share toward ~55–58% by 2028.

---

## Section 4 — Revenue Mix & Business Model

| Stream | FY2025 ($B) | % of Revenue | Gross Margin | Key Driver |
|--------|------------|-------------|--------------|------------|
| Merchant Solutions | ~$8.4 | ~73% | 36.8% | Payments (+38% GPV), shipping, capital, Shop Campaigns (2× revenue) |
| Subscription Solutions | ~$3.2 | ~27% | 81% | SaaS plans, Plus upgrades (34% of MRR), enterprise wins |
| **Total** | **$11.56** | **100%** | **48.2% blended** | |

**Q4 2025 details**:
- Shopify Payments GPV $84B (+38% YoY)
- Payments penetration 68% of GMV (up from 64%)
- Shop Pay GMV growth +62% YoY
- MRR ~$210M, +15% YoY; Plus MRR is 34% of total

**New 2026 wedge — Agentic Plan**: Shopify is opening Catalog to non-Shopify brands via a standalone "Agentic Plan." Brands can list products in Catalog and access AI commerce channels (ChatGPT, Gemini, Copilot) without migrating their full store. Expands addressable merchant base materially; not yet in any analyst model.

---

## Section 5 — Competitive Moat

**Rating: Wide and Widening**.

| Source | Strength | Evidence |
|--------|----------|---------|
| Switching costs | Primary | Full-stack integration; 11+ Q of 25%+ growth; GM/L'Oréal/Estée Lauder migrations signal enterprise stickiness |
| Platform network effects | Strong | 875M+ unique shoppers; Shop Pay; Shop App native GMV +94% |
| Data / AI advantage | Growing | $378B GMV dataset → Catalog LLMs + Sidekick |
| Agentic commerce first-mover | Emerging | UCP co-developed with Google; 20+ endorsers across payments + retail |

**OpenAI Instant Checkout retreat as moat validation**: OpenAI abandoned native checkout after only ~30 Shopify merchants were successfully onboarded; revised model routes users to Shopify merchant storefronts. This is the cleanest single piece of primary-source validation that fulfillment complexity flows *through* Shopify, not around it.

**Vulnerabilities**: 84× P/E is the moat's tax — any deceleration below 20% growth compresses violently. Amazon Buy with Prime is a real but narrow competitive alternative. Lütke's dual-class control is feature-now, governance-risk-later.

---

## Section 6 — Management & Leadership

**Tobias Lütke (Founder/CEO)**: Founder-owner. Controls 40% voting via dual-class + Founder Share. Pre-arranged 10b5-1 plan to sell ~2M Class A shares through Dec 2026 (routine; consistent with prior plans). Retains 77.75M Class B shares. Long-duration thinker; has refused to abandon platform-purity for short-term margin.

**Harley Finkelstein (President)**: Commercial strategy lead. Public-facing on Shopify's "AI commerce" narrative. "2025 was Shopify at full throttle … 2026 will be the year of the builders."

**Jeff Hoffmeister (CFO)**: Drove OpEx as % of revenue down 3 pts YoY (32% → 29%). Architected the $2B buyback — Shopify's first-ever capital return program.

**Capital allocation**:
- **R&D + sales**: Disciplined — headcount –6% on +30% revenue growth
- **M&A**: Quiet; no large acquisitions in 2025
- **Buyback**: $2B authorized Feb 2026 — significant signal of management's undervaluation view at sub-$130
- **Dividend**: None

**Verdict**: Founder-led, operationally disciplined. Revenue guidance met or exceeded for 11+ consecutive quarters. Governance concern (dual-class with Founder Share) is real but in the founder-control "feature" zone for now.

---

## Section 7 — Strategic Growth Initiatives

1. **Universal Commerce Protocol (UCP)**: Open standard for AI agent commerce. Co-developed with Google; endorsed by Visa, Mastercard, Stripe, Adyen, AmEx, Target, Walmart, Etsy, Wayfair, 20+ others. Supports REST, MCP, A2A, AP2 protocols. The "TCP/IP of commerce."
2. **Agentic Storefronts**: Activated by default for ALL Shopify stores late March 2026 across ChatGPT, Google AI Mode, Gemini, Copilot. AI traffic up 7×, AI-attributed orders up 11–15× since Jan 2025.
3. **$2B Share Buyback**: First meaningful capital return; pre-arranged algorithmic execution. Aggressive deployment at $90–$130 would be a significant capital-allocation signal.
4. **B2B Commerce**: GMV +96% in FY2025 (+84% Q4). Small base; rapid expansion. Enterprise customers consolidating D2C + wholesale.
5. **International expansion**: Nearly half of merchant base outside North America; Europe GMV +49%; Payments in 16 new regions; international revenue +36% (accelerating).
6. **Shop Campaigns advertising**: Revenue doubled, adoption tripled in 2025. High-margin layer leveraging the commerce dataset.
7. **Enterprise / Plus migration**: GM, Sonos, L'Oréal, Benetton, Keurig Dr. Pepper, Amer Sports (Wilson, Salomon). Plus MRR 34% of total.

---

## Section 8 — Key Risks

| Risk | Impact | Probability | Notes |
|------|--------|-------------|-------|
| Revenue deceleration <20% | High | Low–Moderate | Primary thesis-break; 84× P/E does not survive sub-20% |
| Gross margin pressure (mix shift) | Moderate | High | Merchant Solutions growing faster than Subscription; structural and accepted |
| AI disruption to SaaS pricing | High | Low | Shopify is payments + shipping + lending + AI infra, not just SaaS |
| Macro / tariff / SMB weakness | Moderate | Moderate | De minimis removal; recession exposure on SMB |
| Multiple compression | High | Moderate | At ~85× fwd P/E, any broad SaaS reset weighs regardless of fundamentals |
| CEO 10b5-1 stock sales | Low | Certain | ~2M shares through Dec 2026; pre-arranged; Lütke retains 77.75M Class B |
| Competition (Amazon Buy with Prime) | Moderate | Moderate | Real but narrow; lacks merchant-platform breadth and AI positioning |

**Thesis-break signal**: Two consecutive quarters of sub-20% revenue growth + declining margins. Price alert at $85 (re-test of 52-wk low) for thesis reassessment.

---

## Section 9 — Industry-Specific Macro Analysis

**E-Commerce Landscape (2025–2026)**:

| AI Platform | Status (Apr 2026) | Impact on SHOP |
|-------------|-------------------|---------------|
| Google AI Mode / Gemini | UCP-native; no fees | Zero-cost new discovery channel |
| ChatGPT (OpenAI) | Retreated from native checkout; Storefront Apps model | Validates complexity; routes to Shopify |
| Microsoft Copilot | Embedded checkout live | Additional discovery surface |
| Amazon (Rufus / Buy with Prime) | Partial competitive threat | Lacks merchant breadth |

**Key macro insight**: AI is **additive** to Shopify, not substitutive. AI-attributed orders +11–15× since Jan 2025. OpenAI's admission that agentic checkout is "harder than expected" + pivot to routing through Shopify storefronts is the strongest single moat-validation signal in the dataset.

**Macro headwinds**: Tariff regime (de minimis removal hits cross-border SMBs), SMB consumer-confidence sensitivity, SaaS multiple compression from rate environment.

---

## Section 10 — Valuation & Comparable Analysis

### Current Multiples (April 17, 2026, $125.83 / ~$163B market cap)

| Metric | Value | Notes |
|--------|-------|-------|
| Market Cap | ~$163B | |
| Trailing P/E | ~93× | $1.36 GAAP EPS |
| FY2026E P/E | ~85× | $1.48 consensus EPS |
| P/S (FY2025) | ~14× | |
| EV/FCF (FY2025) | ~80× | $2.0B FCF; net cash adjusts |
| FY2026E Revenue | ~$15.0B | +30% growth consensus |
| Net Cash | ~$4.6B | |
| Beta | 3.23 | high volatility |

### "100% Return" Math ($126 → $252 in 3–5 years)

Consensus 2027 EPS ~$2.40. At 80× forward P/E → ~$192. To reach $252 requires ~$3.00 EPS at 80×, or ~$4.00 at 60×. Achievable by 2028–2029 if 25%+ growth persists with margin expansion + buyback share-count reduction.

### Comparable Peer Set

| Company | Growth | FY2026E P/E | Notes |
|---------|--------|------------|-------|
| SHOP | +30% | ~85× | Premium for moat + AI positioning |
| WIX | ~12% | ~25× | No payments flywheel |
| BigCommerce | ~5% | n/m | Sub-scale |
| Amazon (full) | ~10% | ~35× | Conglomerate; not pure comp |

**Assessment**: At $125.83 SHOP trades at the top of its peer set on every multiple — the premium is defensible while revenue growth holds 25%+ and the AI-amplification thesis confirms. Compression to 50–60× P/E (Bear case) is the principal risk at this multiple.

**DCF intrinsic value [Estimate — analyst model]**: At 22% revenue CAGR, 18% terminal FCF margin, 25× terminal FCF → $180–$220, ~50–80% above current $125.83.

---

## Section 11 — Catalyst & Sentiment Tracker

### Live Price & Positioning

- **Price**: $125.83 (Apr 17, 2026, Yahoo Finance JSON API)
- **52-week range**: $88.14 – $182.19
- **% from 52-wk high**: –30.9%
- **% above 52-wk low**: +42.8%
- **Market cap**: ~$163B
- **Beta**: 3.23

### Recovery Context

The stock fell from $182 (early 2025) to $88 (recent low) — a 52% drawdown driven by SaaS multiple compression + thematic AI-disruption fear, despite revenue *accelerating* from +26% (FY2024) to +30% (FY2025). The recovery to $126 (+43% off the low) reflects partial re-rating as Q4 2025 results (Feb 11, 2026), the $2B buyback authorization, and Agentic Storefronts traction validated the amplification thesis. The re-rating is mid-flight: consensus targets ($162–$165 median) imply another +29% to median.

### Analyst Consensus (mid-April 2026)

- **Aggregate rating**: Buy (~28 Buy / 9 Hold / 0 Sell as of Q4 2025 readout; expanding toward Buy on recovery)
- **Median price target**: ~$162–$165
- **High target**: ~$220
- **Low target**: ~$110
- **Implied upside from $125.83**: ~+29% to median

### Recent Analyst Actions

| Date | Firm | Action | Note |
|------|------|--------|------|
| Q1 2026 (various) | Multiple | Target raises post-Q4 FY2025 + AI traction | Aggregate median crept from $148 → $162+ |
| 2026-02-18 | Truist | Upgrade | Triggered +8.5% rally on day |
| 2026-02-11 | Multiple | Mixed reaction to Q4 print | Stock fell 6–13% despite top-line beat |

### Short Interest

- **Latest [Estimate]**: ~3% of float (low for a high-beta name)
- **Interpretation**: Recovery off $88 likely flushed shorts; not a "bears piled in" setup. Behavioral signal is more "longs returning" than "bear capitulation."

### Insider Activity (Last 90 Days)

- **Tobias Lütke (CEO)**: Continues 10b5-1 sale program (~2M Class A shares through Dec 2026; pre-arranged; routine). Retains 77.75M Class B. Not interpretable as a directional signal.
- **No notable open-market insider buys or executive cluster sells in the last 90 days.**

### Recent Corporate News (Last 90 Days)

- **[2026-02-11] [Earnings]** — Q4 FY2025 results: Revenue $3.67B (+31%), GMV $123.8B (+31%), FCF $715M (19.5% margin); FY2025: $11.56B (+30%), $378B GMV, $2B FCF [Source: IR]
- **[2026-02 (mid)] [Capital]** — $2.0B share buyback authorized (first-ever capital return) [Source: IR]
- **[Late March 2026] [Strategic]** — Agentic Storefronts activated by default across all Shopify stores (ChatGPT, Google AI Mode, Gemini, Copilot) [Source: company blog / IR]
- **[2026-02-18] [Analyst]** — Truist upgrade triggered +8.5% rally [Source: market reports]
- **[Q1 2026]** — Recovery from $88.14 low to ~$126 (+43%) on multiple compression unwinding + buyback announcement

### Upcoming Catalysts

| Date | Event | Why It Matters |
|------|-------|---------------|
| **~May 2026** | **Q1 2026 earnings** | Tests "low-30s%" revenue growth guidance vs. street ~25%; first quarter with Agentic Storefronts default-on; buyback execution pace |
| Ongoing | Agentic Plan adoption (non-Shopify brands) | New revenue stream not in any analyst model |
| Ongoing | Buyback execution | Aggressive deployment at $90–$130 = significant signal |
| FY2026 (rolling) | International revenue growth (+36% trajectory) | Confirms global expansion thesis |

---

## Section 12 — BAIT Framework

### B — Behavioral: **Moderate** (was Strong at $117–121 trough)

The original triple-overlap thesis hinged on the SaaS-selloff + AI-disruption-fear narrative driving SHOP from $182 to $88 while revenue *accelerated* from 26% to 30%. The recovery to $125.83 has unwound roughly half the sentiment dislocation. Fear is no longer the dominant emotion — partial re-rating is in progress. The Behavioral signal has *moderated* from Strong (at $117) to Moderate (at $126). It would re-strengthen on any drawback toward $100–$110.

### A — Analytical: **Moderate-Strong**

Consensus continues to underweight:
- **Attach-rate compounding** (revenue grows faster than GMV permanently as Payments/shipping/capital/ads layer in)
- **Agentic Plan TAM expansion** to non-Shopify brands — not in any analyst model as of April 2026
- **Operating leverage** from flat headcount on +30% revenue growth (OpEx % of revenue 32% → 29%)
- **$2B buyback EPS accretion** — first-ever capital return is structurally underweighted

DCF at 22% CAGR / 18% terminal FCF margin / 25× terminal multiple → $180–$220 [Estimate]. Consensus median $162–$165 is conservative on the same-ish inputs.

### I — Informational: **Moderate**

Underappreciated primary-source data:
1. **OpenAI Instant Checkout retreat** (only ~30 merchants successfully onboarded; routes users to Shopify storefronts) — clearest single moat-validation signal
2. **AI-attributed orders +11–15× since Jan 2025**
3. **UCP endorsement breadth**: Visa, Mastercard, Stripe, Adyen, AmEx, Target, Walmart, Etsy, Wayfair, 20+ — payments layer rallying around Shopify's standard
4. **Agentic Plan revenue stream** in zero analyst models

### T — Technical: **Moderate**

- $2B buyback creates structural demand floor
- Recovery from $88 to $126 broke the downtrend; stock now testing $130 resistance
- Truist upgrade triggered +8.5% rally — positive breadth
- CEO 10b5-1 sales create mild but pre-priced supply overhang
- Beta 3.23 — high volatility cuts both ways

### BAIT Verdict

**Triple overlap (B + A + I) — recovered conviction**. The Behavioral signal has moderated post-recovery (from Strong → Moderate); Analytical and Informational signals remain Moderate-Strong; Technical is Moderate. Conviction: **Moderate-High**. The cleanest entry zone has moved up from the prior $117–121 to **$110–125** — current price sits at the *upper edge* of the attractive entry zone. A drawback to $100–$110 on broad-market weakness would re-create a Strong-conviction setup; the current price is a defensible-but-not-fat entry.

---

## Section 13 — Bull / Bear / Base Cases

*All scenarios are 3-year (2028E EPS basis) terminal prices. Probabilities sum to 100%.*

| Scenario | 2028E EPS | P/E | Price Target | Return from $125.83 | Probability |
|----------|----------|-----|--------------|---------------------|-------------|
| Bear | $2.50 | 45× | $113 | –10% | 15% |
| Base | $3.50 | 55× | $193 | +53% | 45% |
| Bull | $4.20 | 60× | $252 | +100% | 30% |
| Bull+ | $5.00 | 65× | $325 | +158% | 10% |

### Bear Case ($113) requires
- Revenue growth decelerates to 12–15%; SMB merchant churn
- AI tools commoditize e-commerce; subscription pricing erodes
- Amazon Buy with Prime gains material GMV
- Macro recession + tariffs hit consumer spending
- Multiple compresses to 40–50×

### Base Case ($193) requires
- Revenue sustains +25% via attach rate + international + B2B
- Operating margins reach 16–17%
- Buyback retires 3–5% of share count
- Multiple holds 55× (in line with current premium-growth-tech median)

### Bull Case ($252) requires
- UCP becomes industry standard; Catalog the default AI product-discovery layer
- Revenue sustains +28–30% via attach rate + international + B2B + Agentic Plan
- Operating margins reach 18–20%
- FCF reaches $4–5B; buyback retires 5–8% of share count

### Bull+ Case ($325) requires
- All Bull conditions plus Agentic Plan adds hundreds of thousands of non-Shopify merchants
- AI-attributed orders become 10%+ of GMV by 2028
- Multiple expands to 65×

---

## Section 14 — Probability-Weighted Expected Value

| Scenario | Price | Probability | Weighted Contribution |
|----------|-------|-------------|----------------------|
| Bear | $113 | 15% | $16.95 |
| Base | $193 | 45% | $86.85 |
| Bull | $252 | 30% | $75.60 |
| Bull+ | $325 | 10% | $32.50 |
| **PW EV (3-yr)** | | **100%** | **~$212** |

| Current Price | PW EV | Return (3-yr) | Annualized |
|--------------|-------|--------------|-----------|
| $125.83 | $212 | **+68%** | ~+19%/yr |

**Interpretation**: PW EV of ~$212 vs. spot $125.83 = +68% upside over 3 years (~19%/yr). Asymmetry is favorable: Bull/Bull+ combined probability 40% offers +100% to +158%; Bear (15% probability) implies –10%. Risk/reward roughly **5:1 favorable**, marginally less attractive than at the $117 trough but still compelling.

---

## Section 15 — Recommendation & Bottom Line

**Thesis in one sentence**: Shopify is the dominant independent commerce-infrastructure platform with widening switching costs, accelerating revenue growth (+30% at $11.6B scale), $2B FCF, the first-ever buyback, and a first-mover position in agentic AI commerce that the market has *partially* re-rated from disruption-fear pricing — recovery from $88 to $126 leaves room for another 50%+ if Q1 2026 confirms low-30s% growth.

**For a non-holder**: **Initiate** at $125.83 — current price sits at the upper edge of the attractive entry zone. Asymmetry is favorable (PW EV $212, ~+68% over 3 years; ~5:1 risk/reward). The stronger setup would be a drawback to $100–$110, but waiting for it forfeits the Q1 2026 catalyst optionality.

**For a current holder**: **Add / Hold** — no reason to trim at $126 with PW EV ~$212. Add aggressively on any drawback toward $100–$110. The thesis has *strengthened* since the v1 ingest at $117–121: the Q4 FY2025 print confirmed +30% growth, the $2B buyback was authorized, and the recovery off $88 demonstrated that the Bear case (revenue deceleration to 12–15%) is not appearing in operating data.

**Attractive entry zone**: **$100 – $125** (rationale: 60–80× FY2026E P/E; below DCF intrinsic $180–220; revisit-the-low optionality; original v1 entry zone was $117–121 — recovery has shifted the upper bound up modestly)

**Trim zone**: **$185 – $220** (rationale: top of analyst range; approaches DCF intrinsic mid-point; further upside becomes story-dependent)

**Exit / avoid zone**: **>$280** (rationale: would imply 90×+ FY2026E P/E with no margin of safety; prior $182 high already required pricing in much of the AI-amplification thesis)

**Thesis-break triggers** (would force re-rating toward Avoid):
- Two consecutive quarters of sub-20% revenue growth
- FCF margin compression below 14%
- Payments penetration declining (currently 68%; downtick = attach-rate flywheel breaking)
- Material U.S. e-commerce share loss to Amazon Buy with Prime (below 12%)
- Buyback paused or canceled before $1B executed
- Lütke's voting control modified or Founder Share unwound (signals founder departure path)
- AI-attributed orders growth decelerates from current +11–15× pace to flat/negative

**Next review trigger**: **Q1 2026 earnings — ~May 2026**. Key items: (1) revenue growth vs. low-30s% guide, (2) Agentic Storefronts adoption metrics, (3) buyback execution pace YTD, (4) international revenue growth (continuation of +36% trajectory), (5) any FY2026 guide revision.

---

*Sources: Shopify Q4 FY2025 earnings release (Feb 11, 2026); FY2025 10-K [Estimate of filing date — to verify]; raw/analyses/SHOP_analysis_2026-03-24.html (full 15-section thesis); raw/analyses/shop_analysis.html (Q4 2025 review); Yahoo Finance JSON API (live price 2026-04-17 verified); MarketBeat / TipRanks (analyst consensus); Truist Securities upgrade note (2026-02-18); company IR communications on UCP, Agentic Storefronts, $2B buyback.*
